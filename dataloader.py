...
def _make_dataset(dir):
    ...
    for index, folder in enumerate(os.listdir(dir)):
        ...
        for image in natsorted(os.listdir(clipsFolderPath)):
            # Add path to list.
...
def _pil_loader(path, cropArea=None, resizeDim=None, frameFlip=0):
    ...
    with open(path, 'rb') as f:
        ...
        #resized_img = img.resize(resizeDim, Image.BICUBIC) if (resizeDim != None) else img
...
class SuperSloMo(data.Dataset):
    ...
    def __init__(self, root, transform=None, train=True): # ※ランダム切り出しを削除
        """
        Parameters
        ----------
            root : string
    ...
    def __getitem__(self, index):
        ...
        if (self.train):
            ...
            # Apply random crop on the 9 input frames(関係なし)
            ...
            # Random flip frame(画像の左右反転をランダムで変更)
